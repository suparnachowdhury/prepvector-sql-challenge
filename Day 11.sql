...
select * from song_plays;
	...
	WITH earliest_songs AS (
	SELECT
	  user_id , song_id , MIN(played_at) played_at
	FROM song_plays
	GROUP BY user_id, song_id), ranked_songs AS(
	SELECT 	*
		, DENSE_RANK() OVER(PARTITION BY user_id ORDER BY played_at) rn
	FROM earliest_songs)
	SELECT  
		u.username , r.song_id, r.played_at
	FROM ranked_songs r 
	JOIN users u 
	ON u.id = r.user_id
	WHERE rn = 3
